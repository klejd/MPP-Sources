...
public class Problem1 {
	...
	//implement
				    //return accts.stream().map(x ->x.getCustomer())
				    		//.filter(x ->x.Customer::getCheckingAccount.getId().length() == x.getCustomer().getAddress().getCity().length()).collect(Collectors.toList());
	return accts.stream().map(x->x.getCustomer())
	.sorted(Comparator.comparing(Customer::getLastName))
	.filter(x ->x.getCheckingAccount().getId().length() == x.getAddress().getCity().length())
	.collect(Collectors.toList());
	}
	
	
	//A lis$t of Customers, sorted by last name, 
	...
	public static List<Customer> specialAccounts(List<Customer> custs) {
		...
		.filter(x ->x.getFirstName().startsWith("A")).sorted(Comparator.comparing(Customer::getLastName)).collect(Collectors.toList());
		//implement          
	}
